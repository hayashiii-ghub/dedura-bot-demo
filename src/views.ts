...
export const timeFigure = (minutes: number) => {
  const hours = Math.floor(minutes / 60), rest = minutes % 60
  return hours ? `${hours}<small>時間</small>${rest ? `${rest}<small>分</small>` : ''}` : `${rest}<small>分</small>`
}
...
function home(s: State, ym: string) {
  if (ym !== MONTH) return `<div class="now home-past compact">${monthPeriod(ym)}<div class="mid"><div>打刻は今月でできます</div><a class="ghost" href="#/">今月に戻る</a></div></div>${botList(s, '', false, ym)}<p class="home-demo-note">サンプルの記録は2026年9月にあります。</p>`
  const t = totals(s), todayWork = s.works.filter(w => w.day === TODAY).at(-1)
  const front = s.clock
    ? `<div class="mid"><div class="clock-value"><div class="fig"><small>経過 </small><span data-elapsed>${timeFigure(0)}</span></div><div class="clock-detail"><i class="live" aria-hidden="true"></i><span class="clock-site" title="${e(s.clock.site)}">${e(s.clock.site)}</span><span>${s.clock.start}–</span></div></div>${button('退勤', 'clock-out')}</div>`
    : todayWork
      ? `<div class="mid"><div class="clock-value"><div class="fig"><small>実働 </small>${timeFigure(todayWork.minutes)}</div>${todayWork.minutes > 480 ? `<div class="clock-overtime">残業 ${timeFigure(todayWork.minutes - 480)}</div>` : ''}<div class="clock-detail"><span class="clock-site" title="${e(todayWork.site)}">${e(todayWork.site)}</span><span>${todayWork.start}–${todayWork.end}</span></div></div><a class="ghost" href="#/kintai">勤務を確認</a></div>`
