...
test('ホームだけで月を移り、過去月の記録なしとbotへの月引継ぎを表示する', async ({ page }) => {
  ...
  await expect(page.locator('.home-period .month')).toHaveText('2026年8月')
  ...
  if ((page.viewportSize()?.width ?? 0) >= 900) await expect(page.locator('.desktop-sidebar-top .month')).toHaveText('2026年8月')
...
test('裏面の日付から月を選び、月選択ではカードが返らない', async ({ page }) => {
  await page.goto('/')
  await page.getByRole('button', { name: '今月のつみあげを見る' }).click()
